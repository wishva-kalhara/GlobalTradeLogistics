...
Every cross-cutting concern in this codebase (auth, authorization, audit logging, idempotency, **step-by-step trace logging**) is a **custom annotation + interceptor pair** (or a JAX-RS filter/mapper for the api-gateway layer), not a framework feature (`@RolesAllowed`, JAAS, CDI interceptor bindings). This is a deliberate teaching choice for the assignment this project implements, not an accident — see each annotation's javadoc for why.

Companion to [`RBAC.md`](./RBAC.md) (who can call what), [`EXCEPTIONS.md`](./EXCEPTIONS.md) (what a rejected call returns), and [`TRACE_LOGGING.md`](./TRACE_LOGGING.md) (the `LogEvent` breadcrumb trail every layer emits).
...
- Fires `LogEvent` traces: `TRACE` on successful authentication (correlation key = email), `WARN` on missing bearer token or rejected token (correlation key = request path). See [`TRACE_LOGGING.md`](./TRACE_LOGGING.md).
...
- The interceptor also fires `LogEvent` `WARN` lines before throwing — `"RequiresRoleInterceptor: no authenticated principal for …"` or `"RequiresRoleInterceptor: role X denied for …"` — so auth denials show up in the trace tail even when no controller method ran.
...
- On a successful audit publish, the interceptor also fires a `LogEvent` `TRACE` line (`"AuditInterceptor: published audit for …"`) — see [`TRACE_LOGGING.md`](./TRACE_LOGGING.md).
...
- `IdempotencyInterceptor.checkIdempotency()`: before running the business method, checks `IdempotencyKeyRegistry` for that key. Already seen → short-circuits, returning `null` without calling the business method at all, and fires a `LogEvent` `TRACE` line (`"IdempotencyInterceptor: short-circuiting duplicate key for …"`). First-seen → proceeds, then records the key in the in-memory registry after a successful return.
- Keys are **not** persisted to the database — the registry is process-wide and resets on restart. Duplicate-key protection works immediately in dev and prod without `IS_PROD=true`.
...
4. Document it here, and cross-reference from `RBAC.md`/`EXCEPTIONS.md`/`TRACE_LOGGING.md` if it changes who-can-do-what, what a rejected call returns, or what shows up in the live trace tail.
